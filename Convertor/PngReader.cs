...
namespace Convertor
{
    public class PngReader 
    {
        ...
        public IImageFormat ReadImage(string fileName)
        {
            ...
            IImageFormat image = new Png
            {
                Width = header.Width,
                Height = header.Height,
                _Pixels = GetPixelMatrix(unfilteredData, header)
            };
            return image;
        }
            ...
            for (int i = 0; i < pngHeader.Height; i++)
            {
                ...
                for (int l = 0; l < pngHeader.Width; l++)
                {
                    pixels[i][l] = pngHeader.ColorType == 2
                        ? new Pixel
                        {
                            Red = unfilteredData[(starOfRow + 1) + (bytesPerPixel * l)],
                            Green = unfilteredData[(starOfRow + 2) + (bytesPerPixel * l)],
                            Blue = unfilteredData[(starOfRow + 3) + (bytesPerPixel * l)],
                            Alpha = 0
                        }
                        : new Pixel
                        {
                            ...
                            Green = unfilteredData[(starOfRow + 2) + (bytesPerPixel * l)],
                            Blue = unfilteredData[(starOfRow + 3) + (bytesPerPixel * l)],
                            Alpha = unfilteredData[(starOfRow + 4) + (bytesPerPixel * l)]
                        };
                }
            }
            return pixels;
        ...
        private List<PngChunk> ReadChunks(byte[] array)
        {
            ...
            for (int i = 8; i < array.Length;)
            {
                ...
                {
                    PngChunkType = DeterminePngChunkType(chunkName),
                    ChunkBody = array.Skip(8 + i).Take(chunkSize).ToArray()
                };
                chunks.Add(chunk);
                i += chunkSize + 8 + 4;
            }
